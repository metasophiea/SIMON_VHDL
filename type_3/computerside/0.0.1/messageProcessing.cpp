std::string processMessage(bool mode, unsigned int method, std::string key, std::string message){
	...
	if(!writeMode_clockAndLoad(mode)){std::cout << "- error:processMessage: failed to write mode and load" << std::endl; return "process failure";}

	//push through registers
		for(unsigned int a = 0; a < registerCount+1; a++){
